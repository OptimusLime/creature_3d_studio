//! Phase 16 Uniform Stepped: Test with UNIFORM color stepped terrain
//!
//! Uses uniform color so greedy meshing can merge large areas.
//! This should reveal if the AO artifacts are in greedy merging.
//!
//! Run with: `cargo run --example p16_uniform_stepped`
...
const SCREENSHOT_PATH: &str = "screenshots/p16_uniform_stepped.png";

fn main() {
    std::fs::create_dir_all(SCREENSHOT_DIR).expect("Failed to create screenshots directory");

    println!("Running Phase 16 UNIFORM STEPPED: Greedy meshing with uniform color stepped terrain...");

    App::new()
        .add_plugins(DefaultPlugins.set(WindowPlugin {
            primary_window: Some(Window {
                resolution: (1024, 768).into(),
                title: "Phase 16: Uniform Color Stepped".into(),
                ..default()
            }),
            ..default()
        }))
...
/// Create world with UNIFORM COLOR stepped terrain.
fn create_world() -> VoxelWorld {
    let mut world = VoxelWorld::new();

    // Single uniform color per chunk - NO height variation in color
        ...
        for cz in 0..=1 {
            ...
            for lx in 0..CHUNK_SIZE as i32 {
                for lz in 0..CHUNK_SIZE as i32 {
                    ...
                    for wy in 0..height {
                        // UNIFORM COLOR - no height factor!
...
fn setup(
    ...
    println!(
        "Spawned {} chunk meshes + {} point lights",
