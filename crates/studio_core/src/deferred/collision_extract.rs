...
pub fn extract_terrain_occupancy_system(
    ...
    if extracted.chunks.is_empty() {
        if let Some(terrain) = terrain.as_ref() {
            ...
            if !extracted.chunks.is_empty() {
                info!(
                    "Extracted {} terrain chunks for GPU collision",
...
mod tests {
    ...
    fn test_aabb_extraction_produces_empty_occupancy() {
        ...
        assert!(
            extracted.occupancy_data.is_empty(),
            "AABB should have empty occupancy data"
        );
        assert!(extracted.is_aabb, "Should be marked as AABB");
        assert_eq!(
            extracted.rotation,
            Quat::IDENTITY,
            "AABB should have identity rotation"
        );
    }
    ...
    fn test_aabb_voxel_positions_for_collision() {
        ...
        assert!(
            (world_pos_0.y - 4.5).abs() < 0.001,
            "Bottom voxel center should be at y=4.5, got {}",
            world_pos_0.y
        );
        ...
        assert!(
            (world_pos_1.y - 5.5).abs() < 0.001,
            "Top voxel center should be at y=5.5, got {}",
            world_pos_1.y
        );
    ...
    fn test_correct_aabb_size_for_collision() {
        ...
        assert_eq!(
            correct_size,
            UVec3::new(2, 3, 2),
            "Correct size with boundary padding"
        );
