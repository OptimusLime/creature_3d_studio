...
use super::point_light_shadow::{
    PointShadowBindGroups, PointShadowPipeline, ShadowCastingLights,
    ViewPointShadowTextures, POINT_SHADOW_MAP_SIZE,
};
...
impl ViewNode for PointShadowPassNode {
    ...
    fn run<'w>(
        ...
        info_once!("Point shadow pass: rendering {} lights", shadow_lights.lights.len());
        ...
        for (light_idx, _light) in shadow_lights.lights.iter().enumerate() {
            ...
            for face_idx in 0..6 {
                ...
                let Some(view_bind_group) = bind_groups.view_bind_groups.get(view_bind_group_idx) else {
                    continue;
                };
