...
impl GpuWorldOccupancy {
    ...
    pub fn new(render_device: &RenderDevice, max_chunks: u32) -> Self {
        ...
        let chunk_texture = render_device.wgpu_device().create_texture(&wgpu::TextureDescriptor {
        ...
        let chunk_index_buffer = render_device.wgpu_device().create_buffer(&wgpu::BufferDescriptor {
...
impl GpuCollisionResult {
    ...
    pub fn contacts_for_fragment(&self, fragment_index: u32) -> impl Iterator<Item = &GpuContact> {
        self.contacts.iter().filter(move |c| c.fragment_index == fragment_index)
    }
...
impl GpuCollisionPipeline {
    ...
    pub fn new(
        ...
        let contact_buffer = render_device.wgpu_device().create_buffer(&BufferDescriptor {
        ...
        let contact_count_buffer = render_device.wgpu_device().create_buffer(&BufferDescriptor {
        ...
        let readback_buffer = render_device.wgpu_device().create_buffer(&BufferDescriptor {
        ...
        let count_readback_buffer = render_device.wgpu_device().create_buffer(&BufferDescriptor {
        ...
        let uniform_buffer = render_device.wgpu_device().create_buffer(&BufferDescriptor {
        ...
        let fragment_occupancy_buffer = render_device.wgpu_device().create_buffer(&BufferDescriptor {
    ...
    pub fn is_ready(&self, pipeline_cache: &PipelineCache) -> bool {
        pipeline_cache.get_compute_pipeline(self.pipeline_id).is_some()
    }
}
...
mod tests {
    ...
    fn test_gpu_contact_size() {
        ...
        assert_eq!(std::mem::size_of::<GpuContact>(), 32);
    }
    ...
    fn test_chunk_to_texture_data_conversion() {
        ...
        assert_eq!(texture_data[0 * 32 + 0], (1 << 0) | (1 << 31), "texel(0,0) should have bits 0 and 31");
        assert_eq!(texture_data[10 * 32 + 5], 1 << 15, "texel(5,10) should have bit 15");
        assert_eq!(texture_data[31 * 32 + 31], 1 << 0, "texel(31,31) should have bit 0");
    ...
    fn test_texture_lookup_formula() {
        ...
        let test_cases = [
            (IVec3::new(0, 0, 0), IVec3::new(0, 0, 0), UVec3::new(0, 0, 0)),
            (IVec3::new(31, 31, 31), IVec3::new(0, 0, 0), UVec3::new(31, 31, 31)),
            (IVec3::new(32, 0, 0), IVec3::new(1, 0, 0), UVec3::new(0, 0, 0)),
            (IVec3::new(33, 34, 35), IVec3::new(1, 1, 1), UVec3::new(1, 2, 3)),
            (IVec3::new(-1, 0, 0), IVec3::new(-1, 0, 0), UVec3::new(31, 0, 0)),
            (IVec3::new(-32, 0, 0), IVec3::new(-1, 0, 0), UVec3::new(0, 0, 0)),
        ];
        ...
        for (world_pos, expected_chunk, expected_local) in test_cases {
            ...
            assert_eq!(chunk_coord, expected_chunk, "Chunk coord mismatch for {:?}", world_pos);
            assert_eq!(local_pos, expected_local, "Local pos mismatch for {:?}", world_pos);
        }
    }
    ...
    fn test_gpu_collision_result_resolution_vector() {
        ...
        // Add some contacts for fragment 0
        ...
        assert!((res0.y - 0.5).abs() < 0.001, "Y push should be max(0.3, 0.5) = 0.5");
